...
function translateMemberError(
  ...
  if (input.kind === "subagent_type" || (!hasKind && hasSubagentType)) {
    ...
    if (typeof input.subagent_type !== "string" || !agentEligibilityRegistry[input.subagent_type]) {
      return new MemberValidationError(
        `Unknown subagent_type '${subagentType}'. Available ELIGIBLE agents: sisyphus, atlas, sisyphus-junior, hephaestus and meidocho (if D-36 applied). Use delegate-task for read-only agents like oracle, librarian, explore, metis, momus, multimodal-looker.`,
        name,
        "unknown-subagent",
      )
    }
