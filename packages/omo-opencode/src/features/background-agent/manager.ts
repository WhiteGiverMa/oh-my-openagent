  ...
  private async handleSessionErrorEvent(args: {
    ...
    if (sessionId) {
      ...
      if (sessionStillAlive && !isTerminalSessionError(errorInfo)) {
        // Patch 2: 3-strike — count transient session.error events within a rolling window.
        // If the session keeps erroring without recovering, fail the task instead of silently ignoring.
        const now = Date.now()
        ...
        // Fall through to fail the task with a descriptive error
        ...
        // Fall through to fail the task — terminal error means the session will never produce output
      }
    }
